...
describe('buildStreamingMarkdownDisplay - stray <br> tags', () => {
    it('converts <br> tags to real newlines outside code fences for final messages', () => {
        const raw = 'bash<br>rm -rf node_modules<br>rm package-lock.json<br>';
        const display = buildStreamingMarkdownDisplay(raw, {isFinal: true});
        expect(display).toBe('bash\nrm -rf node_modules\nrm package-lock.json\n');
    });

    it('converts <br/> and <br /> variants case-insensitively during streaming', () => {
        const raw = 'Line one<BR/>Line two<br />Line three';
        const display = buildStreamingMarkdownDisplay(raw, {isFinal: false});
        expect(display).toBe('Line one\nLine two\nLine three');
    });

    it('leaves <br> tags inside fenced code blocks untouched', () => {
        const raw = '```html\n<p>Hello<br>World</p>\n```';
        const display = buildStreamingMarkdownDisplay(raw, {isFinal: true});
